...
def chunk_blocks_to_jsonl(
    ...
    CRITICAL: Implements SRS Rule 5 (Disk-First Persistence) with streaming writes.
    Chunks are written to disk one-by-one, NOT accumulated in memory.
    Per SRS §5 High-Precision Mandate: max_tokens = 400 characters (child chunks).
    """
    ...
    # Extract clean document name for heartbeat logs
    # (remove date suffixes like "- August 2025 UK" for cleaner output)
    heartbeat_doc_id = doc_id.split(" - ")[0] if " - " in doc_id else doc_id
    
    # ENFORCE 400-CHAR PRECISION MANDATE (SRS High-Precision)
    if max_tokens != 400:
        sys.stderr.write(
            f"[WARNING] max_tokens={max_tokens}, but SRS mandates 400-char chunks. "
            "Enforcing 400.\n"
        )
    effective_max_tokens = 400  # Non-negotiable per SRS
    ...
    # REQ-CHUNK-6: Hard limit of 512 tokens for text chunks
    HARD_LIMIT_TEXT = 512
    effective_slack = min(CHUNK_SLACK_TOKENS, HARD_LIMIT_TEXT - effective_max_tokens)

    # STREAMING OUTPUT (SRS Rule 5): Open file for writing chunks one-by-one
    output_file = out_path.open("w", encoding="utf-8")
    chunks_written = 0

    def write_chunk_to_disk(chunk: TextChunk) -> None:
        """Write a single chunk to disk immediately (SRS Rule 5)."""
        ...
        # PROGRESS LOGGING: Print heartbeat every 100 chunks
        if chunks_written % 100 == 0:
            try:
                process = psutil.Process(os.getpid())
                ram_mb = process.memory_info().rss / (1024 * 1024)
                sys.stdout.write(
                    f"[DISK-FLUSH] Wrote chunk {chunks_written} for {heartbeat_doc_id} | RAM: {ram_mb:.1f}MB\n"
                )
            except Exception:
                sys.stdout.write(f"[DISK-FLUSH] Wrote chunk {chunks_written} for {heartbeat_doc_id}\n")
            sys.stdout.flush()
    ...
    def flush_buffer() -> None:
        ...
        budget = max(1, (effective_max_tokens + slack) - prefix_tok)

        def _emit_chunk(body: str, ps: int, pe: int) -> None:
            """Emit one or more chunks for a body, writing each to disk."""
            nonlocal chunk_index
            ...
            parts = [b]
            if _estimate_tokens((prefix + b).strip() if prefix else b) > (
                    effective_max_tokens + slack):
            ...
            for part in parts:
                ...
                if tok > effective_max_tokens + slack:
                    tighter = max(1, (effective_max_tokens + slack) - (prefix_tok if prefix else 0))
        ...
        for it in merged:
            ...
            if len(sentences) == 1 and first_sent_tokens > (effective_max_tokens + slack):
                sentences = _split_sentences_relaxed(it["text"])
            if not sentences:
                continue
            ...
            for sent in sentences:
                ...
                for seg in segments:
                    ...
                    if cur_sentences:
                        ...
                        if cand_tok > effective_max_tokens and not (
                            ends_sentence and cand_tok <= effective_max_tokens + slack
                        ):
                            flush_chunk()
                            cand_body = seg
                            cand_text = (
                    ...
                    if cand_tok > effective_max_tokens + slack:
                        flush_chunk()
    ...
    try:
        for block in blocks:
            ...
            if block_type in ("table", "figure"):
                ...
                if not parent and bcrumb:
                    parent = bcrumb[-1]
                heading_lvl = current_heading_level
                if heading_lvl is None and bcrumb:
                    heading_lvl = len(bcrumb)
            ...
            if buffer_tokens >= effective_max_tokens * 6:
                flush_buffer()
        
        flush_buffer()

        # MEMORY HYGIENE (SRS REQ-PDF-05): Explicit garbage collection every 500 chunks
    ...
    sys.stdout.write(f"[COMPLETE] Wrote {chunks_written} total chunks to {out_path}\n")
    sys.stdout.flush()
