...
def extract_figures_from_pdf(
    *,
    ...
    # REQ-MM-01: Hard filtering for meaningful assets only (300x300px minimum)
    min_width_px: int = 300,
    min_height_px: int = 300,
    min_px_area: int = 90_000,
    ...
    try:
        for page_index in range(doc.page_count):
            ...
            for idx, (rx0, ry0, rx1, ry1, area_ratio, _w, _h) in enumerate(
                ...
                # REQ-MM-01: Verify file size meets minimum threshold (10KB)
                file_size_kb = out_img.stat().st_size / 1024
                if file_size_kb < min_file_size_kb:
                    # Delete undersized asset
                    out_img.unlink()
                    continue
