...
def _validate_dpi(value: int) -> int:
    """Validate DPI is within 72-600 range."""
    if value < 72 or value > 600:
        raise typer.BadParameter(f"DPI must be between 72 and 600, got {value}")
    return value
...
def render_pages(
    ...
    dpi: int = typer.Option(
        150,
        "--dpi",
        help="Render-resolutie in DPI (72-600, default 150)",
        callback=_validate_dpi,
    ),
):
    """
...
def ocr_fallback_cmd(
    ...
    lang: str = typer.Option(
        ...
        help=(
            ...
            "or comma-separated list (e.g., 'eng,spa,fra'). "
            "Tesseract format: eng+spa+fra"
        ),
    ),
):
    """
...
def convert_cmd(
    ...
    semantic_overlap: bool = typer.Option(
        ...
        help="Enable dynamic semantic overlap strategy (REQ-CHUNK-7).",
    ),
) -> None:
